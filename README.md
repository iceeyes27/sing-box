...
支持的变量：`SBM_REALITY_PORT`、`SBM_HY2_PORT`、`SBM_SUBSCRIPTION_PORT`、`SBM_REALITY_SNI`、`SBM_NODE_NAME`、`SBM_PUBLIC_IPV4`（直连链接公网 IPv4 覆盖）、`SBM_ARGO_TOKEN` + `SBM_ARGO_DOMAIN`（固定域名模式，需成对提供）、`SBM_ARGO_PROTOCOL`（Argo 隧道传输协议 `http2`/`auto`/`quic`，默认 `http2` 走纯 TCP 443 最稳；出站 UDP 通畅、追求弱网抗丢包可选 `auto`/`quic`）、`SBM_HY2_UP_MBPS` + `SBM_HY2_DOWN_MBPS`（Hysteria2 Brutal 限速，需成对提供，缺省不限速走 BBR）、`SBM_HY2_HOP_RANGE`（Hysteria2 端口跳跃范围，格式 `小端口:大端口`，如 `20000:40000`）、`SBM_CFOPT_AUTO=1`（开启每周 CF 优选自动刷新）。交互模式下这些变量同样生效，作为各提示的默认值。
...
## 🌩️ Argo 隧道：临时域名 vs 固定域名

Argo(cloudflared)隧道支持两种模式，取舍差异很大，按需选择：

| 维度 | 临时隧道 (trycloudflare) | 固定域名 (Named Tunnel + Token) |
| --- | --- | --- |
| 稳定性 | 弱：无 SLA，Cloudflare 会主动断开 / 限流；**每次进程重启都会分配新随机域名**，旧链接随之失效 | 强：域名固定，进程重启后不变，可配合自动重启长期稳定 |
| 是否需要账号 / 域名 | 不需要，匿名即用 | 需要真实 Cloudflare 账号 + 托管在 CF 的真实域名 |
| 身份绑定 / 匿名性 | 匿名，不绑身份 | 域名实名 + 账号，真实身份与链路绑定 |
| 封号爆炸半径 | 小：被断只损失一个随机域名，换一个即可 | 大：被判定滥用可能**连账号带域名一起封停** |
| 触发风控的主因 | 高带宽被限流 | 大流量、跑视频 / 串流、对外分享或当公共代理 |
| 配置成本 | 零 | 需接管域名 DNS、创建 Tunnel、配置 Public Hostname |
| 适用场景 | 临时测试、短期自用 | 追求长期稳定的个人自用 |

**稳定性说明**：临时隧道的"时通时断"很大程度来自重启换域名。脚本已尽量让 cloudflared 进程存活（QUIC 优先 + 自动重连 + 崩溃自动拉起），并在 `sbm links` 时自动抓取当前真实域名；但域名一旦变化，连订阅地址本身都会失效，这是临时隧道无法从服务端根治的限制。追求稳定请用固定域名模式。

> ⚠️ **合规与封号提示**
> - **Cloudflare 侧**：把隧道当通用代理 / 中转属于灰色用法；免费版历史上限制代理大量非 HTML 内容（视频、大文件）。自用低流量风险可控但非零，固定域名被封会连带域名和账号。
> - **法律合规**：是否合规取决于你所在司法辖区，与用不用 Cloudflare 无关。在部分地区搭建 / 使用此类工具本身受限，**向他人提供**风险更高；固定域名的实名属性会让溯源更容易。
> - **降低风险**：若使用固定域名，建议用**专用账号 + 专用域名**隔离爆炸半径，控制流量、不对外分享。追求可控可考虑 VPS 直连（Reality / Hysteria2），不受 Cloudflare 政策影响。
